  ...
  return (
    <section className="card learning-insights" aria-labelledby="insights-title">
      ...
      <div className="grid grid-cols-3 gap-3" style={{ marginTop: '1rem' }}>
        <article><strong>{dueReviews.length}</strong><span>今日到期複習</span><button className="btn btn-outline btn-sm" disabled={!dueReviews.length} onClick={() => onStartRetakeSession(dueReviews)}>開始複習</button></article>
        <article><strong>{favorites.length}</strong><span>收藏題目</span><button className="btn btn-outline btn-sm" disabled={!favorites.length} onClick={() => setCurrentPage('practice-center')}>查看練習</button></article>
        <article><strong>{weakness.recommendation?.category?.replaceAll('_', ' ') || '累積資料中'}</strong><span>{weakness.recommendation?.reason || '至少完成 3 題後提供建議'}</span><button className="btn btn-primary btn-sm" disabled={!weakness.recommendation} onClick={startRecommendation}>推薦練習</button></article>
      </div>
